...
The result closes the fresh one-shot Apply task/thread context. It never calls
Verify, emits a continuation recommendation, or resumes an old task/thread id.
Loop consumes `NEXT: supervisor_join` in the same turn; only the supervisor may
close the applicable wave/ref join and later select Verify. This is not a user
handoff or turn-ending speech act.
